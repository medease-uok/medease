    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              {isStaff && patients.length > 0 && (
                <div className="relative">
                  <label htmlFor="vax-patient" className="sr-only">Search patients</label>
              ...
              <div className="relative flex-1">
                ...
                <label htmlFor="vax-search" className="sr-only">Search vaccinations</label>
                <input
                  id="vax-search"
                  type="search"
                  value={search}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {STATUSES.map((s) => {
                ...
                const count = vaccinations.filter((v) => v.status === s).length
                return (
                  <button
                    key={s}
      ...
      {error && (
        <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-lg" role="alert">
          ...
          <button onClick={() => { setLoading(true); fetchVaccinations() }} className="ml-auto px-3 py-1 text-sm bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors">Retry</button>
        </div>
      )}
      ...
      {!loading && !error && vaccinations.length > 0 && (
        <output aria-live="polite" className="block text-sm text-slate-500">
          {filtered.length} {filtered.length === 1 ? 'vaccination' : 'vaccinations'}
