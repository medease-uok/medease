    ...
    <div className="space-y-6">
      ...
      {conditions.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Active', value: stats.active, color: 'bg-red-100 text-red-700' },
            { label: 'Managed', value: stats.managed, color: 'bg-green-100 text-green-700' },
            { label: 'Monitoring', value: stats.monitoring, color: 'bg-amber-100 text-amber-700' },
          ].map((s) => (
            <Card key={s.label}>
      ...
      {error && (
        <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
          ...
          <p>{error}</p>
          <button onClick={() => { setLoading(true); fetchConditions() }} className="ml-auto text-red-600 font-medium hover:underline">Retry</button>
        </div>
      )}
