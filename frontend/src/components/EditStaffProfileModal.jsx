...
export default function EditStaffProfileModal({ profile, onClose, onSaved, onImageUpdate }) {
  ...
  const validate = () => {
    ...
    if (form.phone.trim() && !PHONE_PATTERN.test(form.phone)) errs.phone = '9 digits starting with 7, 8, or 9';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const submitted = {
        ...form,
        phone: form.phone ? addCountryCode(form.phone) : '',
      };
      ...
      for (const [key, value] of Object.entries(submitted)) {
        let original;
        if (key === 'phone') {
          original = addCountryCode(stripCountryCode(profile[key]));
        } else if (key === 'dateOfBirth' && profile[key]) {
          original = profile[key].split('T')[0];
        } else {
