...
**To update secrets:** Re-run `./scripts/encrypt-secrets.sh`, commit the new `secrets.enc`, and have team members delete `.vault-password` to re-enter the new password.

> **Note:** If `secrets.enc` does not exist, `start.sh` falls back to test keys — no password required.
