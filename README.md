...
├── scripts/
│   └── encrypt-secrets.sh # Admin tool: encrypt secrets into secrets.enc
...
├── start.sh               # Interactive launcher (decrypts secrets, seeding, all services)
...
**To update secrets:** Re-run `./scripts/encrypt-secrets.sh` and commit the new `secrets.enc`.

> **Note:** `.env.development` files are gitignored — they are generated by `start.sh` from `.env.example` + vault secrets. If `secrets.enc` does not exist, `start.sh` uses the defaults from `.env.example`.
