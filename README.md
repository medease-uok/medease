...
├── scripts/
│   ├── lib/vault.js       # Shared crypto utility (OpenSSL-compatible AES-256)
│   └── encrypt-secrets.js # Admin tool: encrypt secrets into secrets.enc
├── secrets.enc            # AES-256 encrypted secrets (committed, safe)
├── docker-compose.yml     # PostgreSQL, Redis, Adminer, Backend, Frontend services
├── start.js               # Interactive launcher (decrypts secrets, seeding, all services)
...
3. Decrypt secrets from `secrets.enc` (first run prompts for the vault password)
4. Build and start all services (PostgreSQL, Redis, Backend, Frontend, Adminer)
...
Secrets (API keys, JWT secret) are stored in `secrets.enc` — an AES-256 encrypted file committed to git. Team members decrypt it at runtime using a shared password.

**Admin setup (one-time):**
```bash
npm run encrypt-secrets
# Enter real secret values + choose an encryption password
# Commit the generated secrets.enc
```
...
**Subsequent runs:** Password is read from `.vault-password` automatically — no prompts.

**To update secrets:** Re-run `npm run encrypt-secrets` and commit the new `secrets.enc`.

> **Note:** `.env.development` files are gitignored — they are generated by the start script from `.env.example` + vault secrets. If `secrets.enc` does not exist, the defaults from `.env.example` are used.
